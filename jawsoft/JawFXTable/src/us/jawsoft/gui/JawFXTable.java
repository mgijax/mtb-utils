...
import us.jawsoft.gui.FXTable.FXDefaultTableModel;
import us.jawsoft.gui.FXTable.FXHeaderRenderer;
import us.jawsoft.gui.FXTable.FXTableModelInterface;
/**
 ...
 * @version 1.0
 */
public class JawFXTable extends JTable
     ...
     */
    public JawFXTable(int rows, int cols) {
        this(new FXDefaultTableModel(rows, cols));
    }
     ...
     */
    public JawFXTable(Object[][] data, Object[] names) {
        this(new FXDefaultTableModel(data, names));
    }
     ...
     */
    public JawFXTable(Vector data, Vector names) {
        this(new FXDefaultTableModel(data, names));
    }
     ...
     */
    public JawFXTable(ArrayList data, ArrayList names) {
        this(new FXDefaultTableModel(data, names));
    }
     ...
     */
    public JawFXTable(FXTableModelInterface model) {
        super(model);
        initColumnSizes();
        initFXHeader();
    }
    ...
    public void mouseReleased(MouseEvent event) {
        ...
        FXTableModelInterface model = (FXTableModelInterface)getModel();
        if (model.isSortable(modelIndex)) {
            debug("Sortable.");
            debug("sortedColumnIndex = " + sortedColumnIndex);
            debug("sortedColumnAscending = " + sortedColumnAscending);
    ...
    /**
     * Specifically added so someonce could extend the FXDefaultTableModel and
    ...
    public Component prepareRenderer(TableCellRenderer renderer,
        ...
        //debug("============ Row Index = " + rowIndex + "====================");
        
        if(getAlternateRowHighlight()) {
            //debug("Row Highlighting is ON");
            if (getRowHiliteColor() != null) {
                //debug("Row Highlight Color is NOT NULL");
                if (rowIndex < (getStartHighlightRow())) {
                    //debug("rowIndex < getStartHighlightRow() [" + rowIndex + " < " +  getStartHighlightRow() + "]");
                    c.setBackground(getBackground());   
                } else {
                    row = rowIndex - (getStartHighlightRow());
                    //debug("rowIndex >= getStartHighlightRow() [" + rowIndex + " >= " +  getStartHighlightRow() + "]");
                    //debug("rowIndex % getAlternateRowHighlightCount() == ??? [" + rowIndex + " % " +  getAlternateRowHighlightCount() + " = " + (rowIndex % getAlternateRowHighlightCount()) + "]");
                ...
                //debug("Row Highlight Color is NULL");
                c.setBackground(getBackground());   
            } 
        } else {
            //debug("Row Highlighting is OFF");
            c.setBackground(getBackground());   
        }
        
        if (isCellSelected(rowIndex, vColIndex)) {
            //debug("ROW IS SELECTED");
            c.setBackground(getSelectionBackground());
        }
        
        //debug("Color set to: " + c.getBackground().toString());
        //debug("=============================================================");
        return c;
    }
    
    public void setAscendingIcon(Icon icon) {
        //FXTableHeader fxHeader = (FXTableHeader)this.getTableHeader();
        //fxHeader.setAscendingIcon(icon);
        this.ascendingIcon = icon;
        
        JTableHeader fxHeader = this.getTableHeader();
        FXHeaderRenderer fxRenderer = (FXHeaderRenderer)fxHeader.getDefaultRenderer();
        fxRenderer.setAscendingIcon(icon);
    }
    ...
    public void setDecendingIcon(Icon icon) {
        ...
        JTableHeader fxHeader = this.getTableHeader();
        FXHeaderRenderer fxRenderer = (FXHeaderRenderer)fxHeader.getDefaultRenderer();
        fxRenderer.setDecendingIcon(icon);
    }
    ...
    public void setModel(FXTableModelInterface model) {
    ...
    public void setHeaderFont(Font headerFont) {
        //FXTableHeader fxHeader = (FXTableHeader)this.getTableHeader();
        JTableHeader fxHeader = this.getTableHeader();
        fxHeader.setFont(headerFont);
    }
    
    public Font getHeaderFont() {
        //FXTableHeader fxHeader = (FXTableHeader)this.getTableHeader();
        JTableHeader fxHeader = this.getTableHeader();
        return fxHeader.getFont();
    }
    
    
    public void displayRowNumbers() {
        FXDefaultTableModel model = (FXDefaultTableModel)this.getModel();
     ...
     */
    protected void initFXHeader() {
        JTableHeader fxHeader = this.getTableHeader();
        fxHeader.setDefaultRenderer(new FXHeaderRenderer(ascendingIcon, decendingIcon));
        //FXTableHeader fxHeader = new FXTableHeader();
        fxHeader.addMouseListener(this);
        //this.setTableHeader(fxHeader);
    }
